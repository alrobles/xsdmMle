...
profile_optim_par <- function(optim_vec, envdat, pa, stepsize = 0.001, parallel = FALSE){
  ...
  f_optim_functor <- function(envdat, pa)function(optim_vec,
    ...
    while(new_optim_ll > thresh){
      ...
      new_optim[parindex] <- new_optim_vec[parindex]
      optim_vec_output_df <- rbind(optim_vec_output_df,
  ...
  if(parallel){
    ...
    res <- purrr::map(1:length(optim_vec), \(x){
      f_test(optim_vec, parindex = x)}, .progress = TRUE)
  }
