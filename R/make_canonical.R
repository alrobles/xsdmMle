#' Title
#'
#' @param bparms  A data frame with parameters
#' to adjust to a canonical form
#'
#' @returns A data.frame with parameters in canonical form
#' 
#' @export
#'
#' @examples
...
make_cannonical <-  function(bparms)
{
  ...
  for (cc in 1:(dim(O)[2]))
  {
    ...
    {
      O[,cc] = -h
      h = sigLtil[cc] #switch the corresponding entries of sigLtil and sigRtil to pay the price 
