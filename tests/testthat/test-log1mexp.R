...
test_that("log1mexp handles infinities and NAs consistently with current code path", {
  x <- c(-Inf, Inf, NA_real_)
  # Negative inputs include -Inf, so we still expect the custom warning;
...
test_that("log1mexp respects a custom cutoff (branch selection) but same math", {
  cf <- 0.1
  x_small <- cf * 0.5  # should use left branch
  x_large <- cf * 2    # should use right branch
