...
test_that("log_prob_detect works with realistic env_dat array", {
  ...
  with_mocked_bindings(
    ...
    {
      # Call the function with return_prob = FALSE (should return log probability)
      result_log <- log_prob_detect(env_dat, mu, sigl, sigr, ctil, pd, o_mat, return_prob = FALSE)
      ...
      result_prob <- log_prob_detect(env_dat, mu, sigl, sigr, ctil, pd, o_mat, return_prob = TRUE)
...
test_that("log_prob_detect handles edge cases for pd", {
  ...
  with_mocked_bindings(
    `like_neg_ltsgr_cpp` = function(env_dat, mu, sigl, sigr, o_mat) 2.0,
    {
      ...
      expect_equal(log_prob_detect(env_dat, mu, sigl, sigr, ctil, 1, o_mat),
                   log(1) - log1pexp(ctil + 2.0))
      
      # pd = 0.5 (50% detection probability)
      expect_equal(log_prob_detect(env_dat, mu, sigl, sigr, ctil, 0.5, o_mat),
