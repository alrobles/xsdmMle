...
test_that("loglik_biol: full coverage including uncovered lines", {
  ...
  expect_error(loglik_biol(1:5, occ, mu, sigl, sigr, ctil, pd, o_mat), regexp = "env_dat")
  expect_error(loglik_biol(env_dat, c(-1, 0), mu, sigl, sigr, ctil, pd, o_mat), regexp = "occ")
  expect_error(loglik_biol(env_dat, occ, mu = NA_real_, sigl, sigr, ctil, pd, o_mat), regexp = "mu")
  expect_error(loglik_biol(env_dat, occ, mu, sigl = NA_real_, sigr, ctil, pd, o_mat), regexp = "sigl")
  expect_error(loglik_biol(env_dat, occ, mu, sigl, sigr = NA_real_, ctil, pd, o_mat), regexp = "sigr")
  expect_error(loglik_biol(env_dat, occ, mu, sigl, sigr, ctil = c(0.1, 0.2), pd, o_mat), regexp = "ctil")
  expect_error(loglik_biol(env_dat, occ, mu, sigl, sigr, ctil, pd = c(0.9, 0.8), o_mat), regexp = "pd")
  expect_error(loglik_biol(env_dat, occ, mu, sigl, sigr, ctil, pd, o_mat = 1:4), regexp = "o_mat")
})
