...
test_that("vsp fails with invalid env_data", {
  ...
  param_list <- list(mu = 1, sigl = 1, sigr = 1, ctil = 1, pd = 1, o_mat = matrix(0, 1, 1))
...
test_that("vsp fails with missing parameters", {
  ...
  bad_params <- list(mu = 1) # Missing required names
  expect_error(vsp(env_data, bad_params, return_raster = FALSE), "param_list must contain")
})
