...
#' pd = 0.9
#' ctil = -2
#' M <- logprobdetect(O, mu, sigRtil, sigRtil, pd, ctil, envdat_ex )
logprobdetect = function(O, mu, sigLtil, sigRtil, pd, ctil, envdat, num_threads = RcppParallel::defaultNumThreads()){
  
  RcppParallel::setThreadOptions(numThreads = num_threads)
  
  h <-  like_neg_ltsgr_cpp(O = O, mu = mu, sigLtil = sigLtil, sigRtil = sigRtil, envdat = envdat)
